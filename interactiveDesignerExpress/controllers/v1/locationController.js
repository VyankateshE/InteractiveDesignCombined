...
//   // ✅ If charts exist → wait for SVG render
//   return Array.from(charts).every(c =>
//     c.querySelector('svg')
//   );
// }, { timeout: 10000 }).catch(() => {
//   console.log("No charts or charts not fully loaded, continuing...");
// });
...
await page.evaluateOnNewDocument((data, name) => {
  try {
    ...
    localStorage.setItem(`common_json_${name}.json`, data);
    localStorage.setItem('common_json_files', name);
    localStorage.setItem('common_json_file_name', name);
    localStorage.setItem('common_json', data);
  } catch (e) {}
}, chartData, chartName);
    ...
    const waitForExpression = req.body?.waitForExpression;
    const renderWaitMs = Number(req.body?.renderWaitMs || 0);
    ...
    if (waitForExpression) {
      await page.waitForFunction(waitForExpression, { timeout: 20000 }).catch(() => {});
    }
